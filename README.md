	

	YOUTUBE Video
	—————————————



	LMAX Disruptor For Concurrency - Youtube 
	————————————————————————————————————————

		Understanding The Disruptor, A Beginner's Guide To Hardcore Concurrency -trisha Gee & Mike Barker - Youtube 

		Concurrent Programming With The Disruptor - Youtube 

		Goto 2015 • Low Latency Java In The Real World: Lmax Exchange & The Zing Jvm • Gil Tene - Youtube 

		Lmax Disruptor And The Concepts Of Mechanical Sympathy - Youtube 

		Concurrent Programming With The Disruptor - Youtube 




	Kevin Bowersox/ Youtube 
	———————————————————————


		Java Classes, Instances And References - Kevin Bowersox/ Youtube 

		Jpa: Self Referencing Relationships - Kevin Bowersox/ Youtube 

		Jpa: @inheritance With Table Per Class - Kevin Bowersox/ Youtube 

		Jpa: Inheritance With @inheritance - Kevin Bowersox/ Youtube 

		Spring Data: Defining Query Methods On Repositories - Kevin Bowersox/ Youtube 

		Criteria Api Introduction - Kevin Bowersox/ Youtube 

		Jpa/jpql: Intermediate Queries With @namedquery - Kevin Bowersox/ Youtube 

		Jpql: The Basics Of The Java Persistence Query Language - Kevin Bowersox/ Youtube 

		Jpa: Inheritance With @mappedsuperclass - Kevin Bowersox/ Youtube 

		Jpa: Ordering Collections With @orderby - Kevin Bowersox/ Youtube 

		Jpa - Element Collections - Kevin Bowersox/ Youtube 

		Jpa: Embedding Embeddables Within Entities - Kevin Bowersox/ Youtube 

		Jpa - Mapping A Many To Many Relationship - Kevin Bowersox/ Youtube 

		Jpa - One To Many Relationship - Kevin Bowersox/ Youtube 

		Jpa One To One Uni/bi Directional Relationship Tutorial - Kevin Bowersox/ Youtube 

		Creating A Maven Archetype - Kevin Bowersox/ Youtube 

		Configuring Spring Data With A Mysql Database - Kevin Bowersox/ Youtube 

		Spring Data Configuration - Kevin Bowersox/ Youtube 




	Distributed Cache 
	—————————————————

		In-process Caching Vs. Distributed Caching - Dzone 

		The Importance Of Distributed Caching - Dzone 

		Distributed Cache – Overview  -  Bozho Techblog





	Event Sourcing, Distributed Systems & Cqrs
	———————————————————————————————————————————


		Intro - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Shortcomings Of Crud - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		The Eventually Consistent Real-world - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Event Sourcing - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Benefits Of Event Sourced Architectures - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Introduction To Event-driven Architectures - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Introduction To Cqrs - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		How To Build An Event Store - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Scalable, Event-driven Coffee Shop - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Implementation In Java Ee - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Using Kafka With Java - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Running The Scalable Coffee Shop - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 

		Outro - Event Sourcing, Distributed Systems & Cqrs - Sebastian Daschner/ Youtube 








	Hypermedia With Java
	————————————————————

		Intro - Rest As In Hypermedia With Java - Sebastian Daschner/ Youtube 

		Why Should We Care - Rest As In Hypermedia With Java - Sebastian Daschner/ Youtube 

		Server-side Implementation, Java Ee & Jax-rs - Rest As In Hypermedia With Java - Sebastian Daschner/ Youtube 

		Client-side For Java - Rest As In Hypermedia With Java - Sebastian Daschner/ Youtube 




	Hazelcast In-memory Data Grid W/ Spring 
	———————————————————————————————————————

		Hazelcast Handson 02 Understanding Backup - Vinayak Jog/ Youtube 

		Hazelcast Handson 01 Creating Instance - Vinayak Jog/ Youtube 

		Hazelcast Handson 03 Member And Client Communication - Vinayak Jog/ Youtube 

		Hazelcast Hands On 04 Cluster Quorum - Vinayak Jog/ Youtube 

		Hazelcast Handson 05 Creating Cluster Groups - Vinayak Jog/ Youtube 

		Hazelcast Handson 06 Hazelcast Mancenter Introduction - Vinayak Jog/ Youtube 





		Spring Ldap Using Ldaprepository - Nishant Nigam/ Youtube 

		Spring Ldap Authentication Using Authenticatedldapentrycontextmapper - Nishant Nigam/ Youtube 

		Spring Ldap Odm - Nishant Nigam/ Youtube 

		Spring Ldap Using Contextmapper - Nishant Nigam/ Youtube 

		Spring Ldap Using Attributemapper - Nishant Nigam/ Youtube 

		Poller In Spring Integration - Nishant Nigam/ Youtube 

		Apachecommonsfiletailingmessageproducer In Spring Integration - Nishant Nigam/ Youtube 

		Pom XML Spring Integration - Nishant Nigam/ Youtube 

		Filewritingmessagehandler In Spring Integration - Nishant Nigam/ Youtube 

		Wiretap In Spring Integration - Nishant Nigam/ Youtube 

		Messageselectinginterceptor In Spring Integration - Nishant Nigam/ Youtube 

		Messagefilter In Spring Integration - Nishant Nigam/ Youtube 

		Aggregator In Spring Integration - Nishant Nigam/ Youtube 

		Messagesplitter In Spring Integration - Nishant Nigam/ Youtube 

		Payloadtyperouter In Spring Integration - Nishant Nigam/ Youtube 

		Expressionevaluatingrouter In Spring Integration - Nishant Nigam/ Youtube 

		Recipientlistrouter In Spring Integration - Nishant Nigam/ Youtube 

		Headervaluerouter In Spring Integration - Nishant Nigam/ Youtube 

		Custom Router In Spring Integration - Nishant Nigam/ Youtube 

		Headerenricher In Spring Integration - Nishant Nigam/ Youtube 

		Transformer In Spring Integration - Nishant Nigam/ Youtube 

		Serviceactivator In Spring Integration - Nishant Nigam/ Youtube 

		Rabbitmq Spring Integration Using Spring Boot - Nishant Nigam/ Youtube 

		Kafka Spring Integration Using Spring Boot Part 2 - Nishant Nigam/ Youtube 

		Kafka Spring Integration Using Spring Boot Part 1 - Nishant Nigam/ Youtube 

		Zuul Fallbackprovider With Spring Boot - Nishant Nigam/ Youtube 

		Zuul Filter With Spring Boot - Nishant Nigam/ Youtube 

		Zuul Apigateway With Spring Boot - Nishant Nigam/ Youtube 

		Redis Messaging With Spring Boot - Nishant Nigam/ Youtube 

		Kafka Streams With Spring Boot - Nishant Nigam/ Youtube 

		Kafka Topic With Multipartition Spring Boot - Nishant Nigam/ Youtube 

		Kafka With Spring Boot - Nishant Nigam/ Youtube 

		Redis Cache With Spring Boot And Mongodb - Nishant Nigam/ Youtube 

		Mongodb With Spring Boot - Nishant Nigam/ Youtube 

		Redis With Spring Boot - Nishant Nigam/ Youtube 

		Rabbitmq With Spring Boot - Nishant Nigam/ Youtube 

		Loadbalancerclient With Eureka Server - Nishant Nigam/ Youtube 

		Feign Client With Eureka Server - Nishant Nigam/ Youtube 

		Eureka Server - Nishant Nigam/ Youtube 

		Spring Cloud Config Server - Nishant Nigam/ Youtube 

		7 Spring Boot Hibernate Using Jpa Repository,@query - Nishant Nigam/ Youtube 

		6 Spring Boot Swagger - Nishant Nigam/ Youtube 

		5 Spring Boot Hibernate Using Jpa Repository - Nishant Nigam/ Youtube 

		4 Spring Boot Hibernate Using Sessionfactory - Nishant Nigam/ Youtube 

		3 Spring Boot Rest Hateoas - Nishant Nigam/ Youtube 

		2 Spring Boot Rest Exception - Nishant Nigam/ Youtube 

		Spring Boot Rest Example - Nishant Nigam/ Youtube 



		Spring Tactics Video - Baeldung/ Youtube 

		Get Started With Jhipster 4 - Youtube/ Matt Raible

		Web Development Using Spring And Angularjs - Youtube/ Christopher Henkel

		Rest-ful Api Design - Youtube/ Spring Developer

		Documenting Restful Apis - Youtube/ Spring Developer

		Understanding Java Garbage Collection And What You Can Do About It - Youtube/ Spring Developer

		Creating Rest-ful, Hypermedia-based Micro-services With Spring Boot - Youtube/ Spring Developer

		How To Create Docker Image And Run Java App (Spring Boot Jar) In A Docker Engine - Youtube/ Tech Primers

		Docker For Java Developers - Intellij Idea/ Youtube 

		Getting Started With Websocket And Server-sent Events Using Java Oracle/ Youtube <https://www.youtube.com/watch?v=vhjz3hftuzu>

		Spring Official Channel /youtube <https://www.youtube.com/user/springsourcedev>

		From Zero To Hero With Spring Websocket <https://www.youtube.com/watch?v=nxakp15cacy>

		Introduction To Websockets <https://www.youtube.com/watch?v=z-cyo1abcp4>

		Intro To Websocket Applications With Spring Framework 4.0 <https://www.youtube.com/watch?v=mjeeavjr2fa>

		Deep Dive Into Spring Websockets <https://www.youtube.com/watch?v=ocac2yow8xk>

		Webinar: Building Websocket Browser Applications With Spring <https://www.youtube.com/watch?v=mmiza3l64ic>







	Maven Series - Chad Darby/ Youtube 
	——————————————————————————————————

		Overview Part - I

		Overview Part - Ii

		Key Concepts - I

		Key Concepts - Ii

		Key Concepts - Iii

		Creating A Simple Project - I

		Creating A Simple Project - Ii

		Creating A Simple Project - Iii

		Maven Repositories Overview

		Maven Repositories Overview

		Local Repository

		Maven Central Repository

		Maven Tutorial #12 - Additional Repositories Overview

		Maven Tutorial #13 - Additional Repositories Demo

		Maven Tutorial #14 - Private Repositories





	Designing And Testing Series  - Youtube/ Google Tech Talks
	——————————————————————————————————————————————————————————

		"The Clean Code Talks -- Unit Testing" - Youtube/ Google Tech Talks

		The Clean Code Talks - Don't Look For Things! - Youtube/ Google Tech Talks

		The Clean Code Talks - "Global State And Singletons" - Youtube/ Google Tech Talks

		"The Clean Code Talks -- Inheritance, Polymorphism, & Testing" - Youtube/ Google Tech Talks

		Design Tech Talk Series Presents: Oo Design For Testability - Youtube/ Google Tech Talks

		How To Write Clean, Testable Code - Youtube/ Google Tech Talks








	Blogs/ Tutorials/ Web-pages 
	———————————————————————————





	Mechanical Sympathy
	————————————————————

		The Lmax Architecture - Martin Fowler 

		Mechanical Sympathy - <https://mechanical-sympathy.blogspot.com/>





	Memory Leak In Java 
	———————————————————


		How Memory Leaks Happen In A Java Application - Stackify

		How Memory Leaks Happen In Java Apps - Dzone 

		Java Enterprise Performance - Dynatrace

		How To Find And Fix Memory Leaks In Your Java Application - Developers.redhat

		Handling Memory Leaks In Java Programs - Ibm 

		Plug Memory Leaks In Enterprise Java Applications - Java World

		Memory Leaks In Java - Code Revisited 

		Ten Things You Need To Know About Java Memory Leak - Karunsubramanian

		Debug A Memory Leak Using Java Flight Recorder - Oracle 

		Java Memory Leak - App Dynamics 

		Memory Leak Testing In Java - Xmatters 

		The Introduction Of Java Memory Leaks - Programcreek








	Advanced Jpa Topics <http://www.objectdb.com/>
	—————————————————————————————————————————————

		Detached Entity Objects

		Locking In Jpa

		Jpa Lifecycle Events

		Shared (L2) Entity Cache

		Jpa Metamodel API




		Jpa Static Metamodel Generator - Jboss Docs 



	Sebastian Daschner
	——————————————————

		Wildfly Kubernetes Exec Probes - Sebastian Daschner

		Dockerized Java Ee 8 Applications With Glassfish 5.0 - Sebastian Daschner

		Jersey Client Dependencies For Jax-rs 2.1 - Sebastian Daschner

		Kubernetes Ide Support With Intellij - Sebastian Daschner

		Acceptance Tests With Wiremock & Kubernetes - Sebastian Daschner

		Zero-downtime Deployment With Kubernetes - Sebastian Daschner

		Maven 3.5.0 Colored Output - Sebastian Daschner

		Inject Kubernetes Configmap Values With Java Ee & Wildfly - Sebastian Daschner

		How To Change Private Static Final Fields - Sebastian Daschner

		How To Change Environment Variables In Java - Sebastian Daschner

		Released Jax-rs Analyzer V0.10 - Sebastian Daschner

		Siren 4 Java Ee Hypermedia Client - Sebastian Daschner

		Designing And Implementing Rest Acceptance Tests - Sebastian Daschner

		Introducing Siren 4 Java Ee 1.0 - Sebastian Daschner

		Enabling Hypermedia Rest With Jax-rs - Sebastian Daschner
		 
		Json Mapping Polymorphism Support - Sebastian Daschner

		Making Java Esoteric — Adding A Maybe Keyword - Sebastian Daschner

		Thoughts On Jsonb (Or One Of The Main Advantages Of Java Ee - Sebastian Daschner

		Create Links To Jax-rs Resources - Sebastian Daschner

		Maven Multiple Module Overhead - Sebastian Daschner

		Utf-8 Charset As Constant In Java - Sebastian Daschner

		Analyze Maven Build Time - Sebastian Daschner

		Making Java Esoteric — Adding A Maybe Keyword - Sebastian Daschner

		Running Java Web Start Apps In A Docker Sandbox - Sebastian Daschner

		Released Jax-rs Analyzer V0.5 - Sebastian Daschner

		Jax-rs Analyzer Explained - Sebastian Daschner

		Released Jax-rs Analyzer V0.4 - Sebastian Daschner

		Log Jpa Schema Creation With Hibernate - Sebastian Daschner

		JPA Joined Subclass Strategy Example - Sebastian Daschner

		JCache With Mapdb As Persistence Provider - Sebastian Daschner

		Simple And Powerful Cache In Javaee - Sebastian Daschner

		Java Collections As Jax-rs Json Response On Tomee - Sebastian Daschner

		Released Jax-rs Analyzer V0.3 - Sebastian Daschner

		Feature List Of Jax-rs Analyzer - Sebastian Daschner

		Released Jax-rs Analyzer - Sebastian Daschner

		Jenkins As A Docker Orchestrator - Sebastian Daschner

		Import Self-signed Ssl Certificate In Java - Sebastian Daschner

		Unorthodox Docker Connection Without Links - Sebastian Daschner






		Apache CXF Spring Restful Services -  Benchresources

		Java/spring Bolg Series - Level Up Lunch 

		Official Spring Guides 

		Code Java Dot Net Tuts  - Codejava.net

		Mkyong Tutorials About The Java/ Spring - Mkyong

		Hellokoding Spring Tutorials

		Java/spring Tutorials - Websystique  

		Java/spring Tutorials - Dzone  







		Learn Java Online With Spring, Hibernate, Webservices & More - Learn-desk.wiziq.com

		Java Persistence - Wikibook

		Spring Batch Ultimate Guide - Java Code Geeks











	Martin Fowler 
	—————————————


		Event Sourcing, Event Architectures - Martin Fowler 

		The Lmax Architecture, Event Sourcing, Event Architectures - Martin Fowler 






	Dzone
	—————

		Connecting Elasticsearch Directly To Your Java Ee Application  






	Micro-services Serices Spring Cloud And Netflix Oss - Callistaenterprise.se
	———————————————————————————————————————————————————————————————————————————


		An Operations Model For Micro-services

		Building Micro-services With Spring Cloud And Netflix Oss, Part 1

		Building Micro-services With Spring Cloud And Netflix Oss, Part 2

		Building Micro-services, Part 3. Secure Api's With Oauth 2.0

		Building Micro-services, Part 4. Dockerize Your Microservices

		Building Micro-services, Part 5. Upgrading To Spring Cloud 1.1 And Docker For Mac

		Building Micro-services, Part 6. Adding A Configuration Server

		Building Micro-services, Part 7. Distributed Tracing With Zipkin And Spring Cloud Sleuth

		Building Micro-services, Part 8. Centralized Logging With The Elk Stack








	Eugen Baeldung 
	——————————————


	Redis
	—————

		Pubsub Messaging With Spring Data Redis

		Introduction To Spring Data Redis

		A Guide To Redis With Redisson

		Introduction To Lettuce – The Java Redis Client

		Intro To Jedis – The Java Redis Client Library

		Guide To Spring Session

		A Guide To Caching In Spring




	AWS Lamda 
	—————————

		Aws S3 With Java

		Aws Lambda With Java

		Aws Lambda Using Dynamodb With Java

		Spring Cloud Aws – S3

		Using Aws Lambda With Api Gateway

		Spring Cloud Aws – Ec2

		Introduction To Aws Serverless Application Model



	Tagging The Words 
	—————————————————

		A Simple Tagging Implementation With Elasticsearch - Baeldung

		A Simple Tagging Implementation With Jpa - Baeldung

		An Advanced Tagging Implementation With Jpa - Baeldung

		A Simple Tagging Implementation With Mongodb - Baeldung




	Scheduling In Spring With Quartz





	Rest With Spring Series 
	———————————————————————


	Rest API Basics - I
	———————————————————


		Bootstrapping A Web Application

		Building A Rest Api

		Securing A Rest Api

		Basic And Digest Authentication For A Rest Api

		Rest Pagination

		Error Handling For Rest

		Entity To Dto Conversion For A Spring Rest API



	Rest API Advanced Topics - Ii
	—————————————————————————————

		Etags For Rest

		Rest Api Discoverability And Hateoas

		Rest Api Discoverability

		Versioning A Rest Api

		Building A Rest Query Language (Series)

		Apply Cqrs To A Spring Rest Api

		Setting Up Swagger 2 With A Spring Rest API




	Test And Consume The Api - Iii
	——————————————————————————————

		Test A Rest Api With Java

		Testing Rest With Multiple Mime Types

		Resttemplate With Basic Authentication

		Resttemplate With Digest Authentication






		Introduction To JCache

		Guide To Hazelcast With Java

		Spring Boot Actuator

		Metrics For Your Spring Rest Api

		Custom Information In Spring Boot Info Endpoint

		An Intro To Spring Cloud Zookeeper

		Setting Up Swagger 2 With A Spring Rest Api

		Spring Boot Authentication Auditing Support




	Concurrency 
	———————————

		Concurrency With Lmax Disruptor – An Introduction

		Java Concurrency Interview Questions (+ Answers)

		Guide To Java.util.concurrent.locks

		A Guide To The Java Executorservice

		Guide To Java.util.concurrent.blockingqueue

		Concurrent Test Execution In Spring 5

		Introduction To Thread Pools In Java

		Control The Session With Spring Security

		A Guide To Concurrentmap

		Custom Thread Pools In Java 8 Parallel Streams

		Guide To The Concurrentskiplistmap

		Guide To Completablefuture

		Guide To Countdownlatch In Java

		Semaphores In Java




	Memory Management 
	—————————————————

		Guide To The Most Important Jvm Parameters

		Memory Management In Java Interview Questions (+answers)

		Jvm Garbage Collectors

		Guide To Java String Pool

		How To Warm Up The Jvm

		Daemon Threads In Java

		How Memory Leaks Happen In A Java Application - Stackify.com





	Design Patterns 
	———————————————

		Introduction To Creational Design Patterns

		Proxy, Decorator, Adapter And Bridge Patterns

		Strategy Design Pattern In Java 8

		The Observer Pattern In Java

		A Guide To The Front Controller Pattern In Java

		Flyweight Pattern In Java

		Integration Patterns With Apache Camel



	High-performance Computing 
	——————————————————————————
	
		Introduction To Ratpack






	Spring Profiles 
	———————————————



	Spring Persistence Series 
	—————————————————————————


	Core Persistence - I
	————————————————————

		Hibernate 3 With Spring

		Hibernate 4 With Spring

		Jpa With Spring

		Persistence With Spring Data JPA



	The Dao - Ii
	————————————

		The DAO With Spring And Hibernate

		The Dao With Jpa And Spring

		Simplify The Dao With Spring And Java Generics



	Advanced Persistence - III
	——————————————————————————

	Transaction Configuration With Spring 4 And Jpa

	Hibernate Mapping Exception – Unknown Entity

	Sorting With Jpa

	Sorting With Hibernate

	Jpa Pagination

	Hibernate Pagination

	Externalize Setup Data Via Csv In A Spring Application

	Use Liquibase To Safely Evolve Your Database Schema

	Dynamodb In A Spring Boot Application Using Spring Data

	Introduction To Liquibase Rollback

	Database Migrations With Flyway

	Auditing With Jpa, Hibernate, And Spring Data Jpa



	Spring Data Mongodb - IV
	————————————————————————

	Introduction To Spring Data Mongodb

	A Guide To Queries In Spring Data Mongodb

	Spring Data Mongodb – Indexes, Annotations And Converters

	Custom Cascading In Spring Data Mongodb

	Introduction To Spring Data Elastic Search 



	Hibernate 
	—————————

	All About Hibernate Second Level Cache - Dzone 

	Hibernate Caching – First Level Cache - Journaldev 

	Hibernate Cache Levels Tutorial - Javacodegeeks 

	Hibernate Second-level Cache

	Guide To Hibernate 4 With Spring

	Hibernate: Save, Persist, Update, Merge, Saveorupdate




	Spring Integration
	——————————————————

	Introduction To Spring Integration

	Getting Started With The Jms 

	Spring Integration - Building A Sample Application - Dzone 



	Apache Solr (Solr Build In The Top Of Apache Lucene)
	————————————————————————————————————————————————————

	Guide To Solr In Java With Apache Solrj

	Introduction To Spring Data Solr

	Full-text Search With Solr



	Zookeeper, Kafka And Akka 
	—————————————————————————

	An Intro To Spring Cloud Zookeeper

	Intro To Apache Kafka With Spring

	Introduction To Spring With Akka

	Guide To Akka Streams

	Guide To Reactive Microservices Using Lagom Framework



	Pagination 
	——————————


	Rest Pagination In Spring

	Hibernate Pagination

	Jpa Pagination

	Pagination With Spring Rest And Angularjs Table

	Guide To Linkrest





	Building A Rest Query Language Series 
	—————————————————————————————————————

	Rest Query Language With Spring And Jpa Criteria

	Rest Query Language With Spring Data Jpa Specifications

	Rest Query Language With Spring Data Jpa And Querydsl

	Rest Query Language – Advanced Search Operations

	Rest Query Language With Rsql

	Rest Query Language With Querydsl Web Support



	JPA
	———

	Rest Query Language With Spring Data Jpa And Querydsl

	Rest Query Language With Spring Data Jpa Specifications

	Sorting With Jpa

	A Guide To Queries In Spring Data Mongodb

	Jpa Pagination

	Spring Jpa – Multiple Databases

	Rest Query Language With Rsql







	A Reddit Application With Spring
	———————————————————————————————


	Authenticating With Reddit Oauth2 And Spring Security

	Post A Link To The Reddit Api

	Schedule Post To Reddit With Spring

	First Round Of Improvements To The Reddit Application

	Rate Limiting Access To The Reddit Api

	Make It Easy To Schedule To Reddit

	Retry To Submit To Reddit A Post Without Enough Traction

	Second Round Of Improvements To The Reddit Application

	Displaying Dates In The Timezone Of The User

	Decoupling Registration From Login In The Reddit App

	Testing The Api Of The Reddit App

	Adding Roles And Privileges To The Reddit App

	Third Round Of Improvements To The Reddit Application

	Entity To Dto Conversion For A Spring Rest Api

	Preserve The History Of Reddit Post Submissions

	Fourth Round Of Improvements To The Reddit Application






	Spring Security Series 
	——————————————————————




	The Registration Series - I 
	———————————————————————————

	The Basic Registration Process

	Registration – Activate A New Account By Email

	Resend The Verification Email

	Registration – Password Encoding

	Reset Your Password



	The Authentication Series - Ii 
	——————————————————————————————

	Basic Form Login

	Form Login – Error Handling And Localization

	Logout

	Redirect To Different Pages After Login

	Remember Me And Persistent Remember Me

	Roles And Privileges

	Prevent Brute Force Authentication Attempts




	Core Spring Security - Iii
	——————————————————————————

	Spring Security With Maven

	Retrieve User Information In Spring Security

	Spring Security Authentication Provider

	Spring Security Expressions – Hasrole Example

	Unable To Locate Spring Namespacehandler For Xml Schema Namespace

	No Bean Named ‘springsecurityfilterchain’ Is Defined

	Spring Security – Security None, Filters None, Access Permitall

	Session Management

	Intro To Spring Security Ldap

	 

	Spring Security With Rest - Iv
	——————————————————————————————

	Securing A Rest Service With Spring Security

	Spring Security – Basic Authentication

	Spring Security – Digest Authentication

	Basic And Digest Authentication For A Rest Api

	Authentication Against A Rest Api With Spring Security

	Resttemplate – Basic Authentication

	Resttemplate – Digest Authentication



	Ldap 
	————

	Intro To Spring Security Ldap

	Spring Ldap Overview

	Guide To Spring Data Ldap


	Oauth
	—————

	Oauth2 For A Spring Rest Api – Handle The Refresh Token In Angularjs

	Simple Single Sign On With Spring Security Oauth2

	Using Jwt With Spring Security Oauth

	Spring Rest Api + Oauth2 + Angularjs

	A Reddit Application With Spring

	Post A Link To The Reddit Api

	Authenticating With Reddit Oauth2 And Spring Security

	Angular 4 Upgrade For Spring Security Oauth

	Spring Rest Api + Oauth2 + Angularjs

	Using Jwt With Spring Security Oauth

	Oauth2 For A Spring Rest Api – Handle The Refresh Token In Angularjs

	Spring Security Oauth2 – Simple Token Revocation

	Oauth 2.0 And Dynamic Client Registration

	Testing An Oauth Secured Api With The Spring Mvc Test Support

	Logout In A Oauth Secured Application

	Oauth2 Remember Me With Refresh Token

	Angular 4 Upgrade For Spring Security Oauth




	Spring Registration And Forms 
	—————————————————————————————

	Spring Security Form Login

	Exploring Springmvc’s Form Tag Library

	Getting Started With Forms In Spring Mvc

	Spring Mvc And The @modelattribute Annotation

	The Registration Process With Spring Security

	File Upload With Spring Mvc

	Introduction To Using Thymeleaf In Spring

	Spring Mvc Custom Validation

	A Guide To Csrf Protection In Spring Security

	Spring Security Registration Tutorial

	The Registration Process With Spring Security

	Registration – Activate A New Account By Email

	Registration With Spring Security – Password Encoding

	Spring Security – Roles And Privileges

	Prevent Brute Force Authentication Attempts With Spring Security

	Spring Security – Reset Your Password

	Spring Security Registration – Resend Verification Email

	The Registration Api Becomes Restful

	Registration – Password Strength And Rules

	Updating Your Password

	Registration - Integrate Recaptcha

	Two Factor Auth With Spring Security

	Registration With Spring – Integrate Recaptcha

	Purging Expired Tokens Generated By The Registration

	Custom Login Page For Returning User

	Allow Authentication From Accepted Locations Only With Spring Security




	Spring Batch 
	————————————


	Introduction To Spring Batch

	Batch Processing With Spring Cloud Data Flow

	Spring Batch Using Partitioner

	Scheduling In Spring With Quartz




	Design Pattern
	——————————————

	Command Query Responsibility Segregation Pattern (Cqrs)
	——————————————————————————————————————————————————————

	Apply Cqrs To A Spring Rest Api

	Cqrs And Event Sourcing Resources

	Guide To Reactive Microservices Using Lagom Framework








	Spring Exceptions Series 
	————————————————————————


	Spring Beans Exceptions - I
	———————————————————————————

	Spring Beandefinitionstoreexception
	———————————————————————————————————

	Causes And Solutions For The Beandefinitionstoreexception In Spring:

	Filenotfoundexception

	Nosuchmethoderror

	Noclassdeffounderror

	Ioexception


	Spring Beancreationexception
	————————————————————————————

	Spring Beancreationexception – Causes And Solutions For:

	Nosuchbeandefinitionexception

	Nouniquebeandefinitionexception

	Beaninstantiationexception

	Notwritablepropertyexception

	Cannotloadbeanclassexception And The Children: Beancurrentlyincreationexception And Beanisabstractexception



	Spring Nosuchbeandefinitionexception
	——————————————————————————————————————

	Causes And Solutions For The  Nosuchbeandefinitionexception In Spring – “no Qualifying Bean Of Type”, “no Bean Named”, Proxied Beans, Etc


	Spring Data Exceptions - Ii
	———————————————————————————

	Spring Dataintegrityviolationexception
	——————————————————————————————————————

	The Spring Dataintegrityviolationexception – Causes And Solutions For Hibernate And Jpa:

	Constraintviolationexception

	Propertyvalueexception

	Dataexception

	Entityexistsexception


	Spring Nontransientdataaccessexception
	——————————————————————————————————————


	Spring Security Exceptions - Iii
	————————————————————————————————

	No Bean Named ‘springsecurityfilterchain’ Is Defined

	No Bean Named ‘springsecurityfilterchain‘ Is Defined – The Problem, The Causes And The Solutions.





	Httpclient 4
	————————————

	Httpclient 4 Tutorial

	Httpclient 4 Cookbook

	Multipart Upload With Httpclient 4

	Httpclient 4 – Send Custom Cookie

	Httpclient 4 – Do Not Follow Redirects

	Httpclient 4 – Setting A Custom User-agent

	Httpclient Basic Authentication

	Posting With Httpclient

	Httpclient 4 – Get The Status Code

	Httpclient With Ssl

	Httpclient Basic Authentication

	Httpclient 4 – Get The Status Code

	Custom Http Header With The Httpclient

	Httpclient 4 – Cancel / Abort Request

	Advanced Httpclient Configuration





	Jackson Json Series
	———————————————————





	Basic Jackson Marshalling - I
	—————————————————————————————


	A Guide To Jackson Annotations

	Ignore Properties On Marshalling

	Ignore Null Fields

	Change Name Of Field

	Intro To The Jackson Objectmapper

	Xml Serialization And Deserialization With Jackson

	Json In Java

	Jackson Annotation Examples

	Jackson – Marshall String To Jsonnode




	Basic Jackson Unmarshalling - Ii
	————————————————————————————————


	Parse Json To Jsonnode

	Unmarshalling Json With Unknown Properties

	Jackson Vs Gson



	Advanced Jackson Marshalling - Iii
	——————————————————————————————————


	Serialize Only Fields That Meet A Custom Criteria With Jackson

	Serialize Enums As Json Objects

	Jsonmappingexception (No Serializer Found For Class)

	Jackson – Custom Serializer

	Jackson Date

	Jackson Json Views

	Jackson Exceptions – Problems And Solutions

	Jackson – Working With Maps And Nulls




	Advanced Jackson Un-marshalling - Iv
	————————————————————————————————————
	 
	Unmarshall To Collection/array

	Jackson – Custom Deserializer



	Advanced Jackson Usage - V
	——————————————————————————

	Jackson – Bidirectional Relationships

	Jackson – Decide What Fields Get Serialized/deserialized

	Working With Tree Model Nodes In Jackson

	More Jackson Annotations

	Inheritance With Jackson






	Redis 
	—————

	Introduction To Spring Data Redis 

	Intro To Jedis – The Java Redis Client Library

	A Guide To Redis With Redisson

	Pubsub Messaging With Spring Data Redis



	Aspectj
	———————

	Intro To Aspectj

	Introduction To Pointcut Expressions In Spring

	Implementing A Custom Spring Aop Annotation




	Introduction To Spring Batch

	Spring Batch Using Partitioner

	Batch Processing With Spring Cloud Data Flow

	Asynchronous Batch Operations In Couchbase





	A Guide To Caching In Spring

	Guide To Spring Session

	Jasperreports With Spring

	Setting Up Swagger 2 With A Spring Rest Api

	Cucumber Spring Integration




	Graph Database 
	——————————————

	A Guide To Neo4j With Java

	Introduction To Spring Data Neo4j

	Introduction To Spring Data Jpa

	Jackson – Bidirectional Relationships





	Cassandra
	—————————

	Introduction To Spring Data Cassandra

	A Guide To Cassandra With Java

	Using The Cassandratemplate From Spring Data

	Data Modeling In Cassandra




	Dynamo-db In A Spring Boot Application Using Spring Data

	Intro To Spring Data Couchbase







	Spring 5 Introduction 

	Introduction To The Functional Web Framework In Spring 5

	Spring 5 Web-client





	Mongodb
	———————

	Spring Data Mongodb Series 

	A Guide To Queries In Spring Data Mongodb

	Spring Data Mongodb: Projections And Aggregations

	Gridfs In Spring Data Mongodb

	Spring Data Mongodb – Indexes, Annotations And Converters

	Custom Cascading In Spring Data Mongodb

	Introduction To Spring Data Mongodb
	 
	A Guide To Mongodb With Java









	Integration Testing In Spring

	Testing In Spring Boot

	Test A Rest Api With Java

	Injecting Mockito Mocks Into Spring Beans

	Quick Guide To @restclienttest In Spring Boot

	Configuring Separate Spring Datasource For Tests

	Webappconfiguration In Spring Tests

	Property Testing Example With Vavr(Formerly Known As Javaslang)

	Self-contained Testing Using An In-memory Database

	Testing With Hamcrest



	Properties With Spring And Spring Boot

	A Guide To Rest-assured



	Spring Security – Cache Control Headers 

	Etags For Rest With Spring

	A Guide To Caching In Spring

	Hibernate Second-level Cache

	Cachable Static Assets With Spring Mvc

	Guava Cache








	Spring Boot 
	———————————


	Spring Boot Dependency Management With A Custom Parent

	Migrating From Spring To Spring Boot

	How To Get All Spring-managed Beans?

	Bootstrap A Simple Application Using Spring Boot

	Spring Boot And Kotlin

	Testing In Spring Boot

	Introduction To The Stripe Api For Java

	Guide To @configurationproperties In Spring Boot

	Create A Custom Auto-configuration With Spring Boot

	Creating A Custom Starter With Spring Boot

	A Guide To Spring Boot Admin

	Dynamic Dto Validation Config Retrieved From The Database

	Using @jsoncomponent In Spring Boot

	Custom Information In Spring Boot Info Endpoint

	Create A Custom Failureanalyzer With Spring Boot

	Exploring The Spring Boot Testresttemplate

	Spring Boot Authentication Auditing Support

	Guide To Internationalization In Spring Boot

	Using Custom Banners In Spring Boot

	Overview Of Spring-boot Dev Tools

	Intro To Building An Application With Spring Boot

	Spring Boot Application As A Service

	Dockerizing A Spring Boot Application

	Quick Guide To @restclienttest In Spring Boot

	Intro To Spring Boot Starters

	Spring Boot Support For Jooq

	Spring And Spring Boot Adoption In March 2016

	Configure A Spring Boot Web Application

	Spring Boot Actuator

	Survey Results – Java 8, Spring 4 And Spring Boot Adoption

	Properties With Spring And Spring Boot






	Entity To Dto Conversion For A Spring Rest Api

	A Guide To Mapping With Dozer

	Quick Guide To Map-struct

	Error Handling For Rest With Spring












	Testing 
	———————

	Integration Testing In Spring

	Testing In Spring Boot

	Intro To Feign

	Rest Api Testing With Cucumber

	Quick Guide To @restclienttest In Spring Boot

	Avoid Brittle Tests For The Service Layer

	Injecting Mockito Mocks Into Spring Beans

	Test A Rest Api With Java






	Guice Dependency Injection 
	——————————————————————————


	Guide To Google Guice

	Ratpack Google Guice Integration

	A Guide To The Java Executorservice

	Guide To Guava’s Eventbus




	Gemfire
	———————

	A Guide To Gemfire With Spring Data

	Intro To Structurizr

	Jackson Unmarshalling Json With Unknown Properties




	Rabbitmq
	————————

	Introduction To Rabbitmq

	Rabbitmq Message Dispatching With Spring Amqp

	Messaging With Spring Amqp

	Spring Remoting With Amqp

	Apply Cqrs To A Spring Rest Api

	Apply Command Query Responsibility Segregation (Cqrs) To A Spring Rest Api

	Introduction To Spring Integration

	Spring Cloud Bus

	Spring Boot Actuator

	Creating A Custom Starter With Spring Boot







	Multi-threading In Java
	———————————————————————


	Introduction To Thread Pools In Java

	Guide To Java.util.concurrent.locks

	A Guide To The Java Executorservice

	Guide To The Concurrentskiplistmap

	How To Do @async In Spring

	Guide To Java.util.concurrent.blockingqueue

	Guide To Synchronized Keyword In Java

	Guide To Java.util.concurrent.future

	Custom Thread Pools In Java 8 Parallel Streams

	Httpclient Connection Management

	A Guide To Concurrentmap

	Guide To Volatile Keyword In Java

	Introduction To Java 8 Streams

	Java Concurrency Interview Questions (+ Answers)





	Web-socket
	—————————

	Intro To Websockets With Spring

	Intro To Security And Websockets

	A Guide To The Java Api For Websocket

	A Java Client For A Websockets Api

	Intro To Security And Websockets

	Spring Rest Api + Oauth2 + Angularjs







	Amazan Web Service (Aws)
	————————————————————————


	Aws S3 With Java

	Aws Lambda With Java

	Aws Lambda Using Dynamodb With Java

	Quick Intro To Spring Cloud Configuration






	Spring Mvc Series 
	—————————————————


	Spring Mvc Tutorial

	Spring Mvc Setup With Kotlin

	Model, Modelmap, And Modelview

	A Guide To The Viewresolver

	Content Negotiation

	File Upload

	A Quick Guide To Matrix Variables

	The @modelattribute Annotation

	Returning Image/media Data

	Using A Custom Handler Interceptor To Manage Sessions

	Custom Validation

	Getting Started With Forms

	Introduction To Handlerinterceptor

	The Httpmediatypenotacceptableexception

	Cachable Static Assets

	Custom Error Pages

	Servlet 3 Async Support With Spring Mvc And Spring Security

	A Custom Data Binder

	Handleradapters

	Upload And Display Excel Files

	Testing An Oauth Secured Api

	Form Validation With Angularjs

	Quick Guide To Spring Mvc With Velocity

	Introduction To Using Freemarker In Spring Mvc

	Spring Mvc + Thymeleaf 3.0: New Features

	Csrf Protection With Spring Mvc And Thymeleaf

	Apache Tiles Integration With Spring Mvc

	Guide To Mustache With Spring Boot

	The Requestbody And Responsebody Annotations In Spring

	Spring Mvc Setup With Kotlin

	Template Engines For Spring

	Spring – Log Incoming Requests

	Intro To Security And Websockets

	Allow Authentication From Accepted Locations Only With Spring Security

	Exploring The Spring 5 Mvc Url Matching Improvements

	Guide To Spring Web Flow

	An Intro To The Spring Dispatcherservlet

	Spring Mvc Custom Validation

	Http Put Vs Http Patch In A Rest Api

	Returning An Image Or A File With Spring

	Form Validation With Angularjs And Spring Mvc

	Testing An Oauth Secured Api With Spring Mvc

	Guide To Spring Webutils And Servletrequestutils

	Upload And Display Excel Files With Spring Mvc

	The @servletcomponentscan Annotation In Spring Boot

	Spring @requestmapping New Shortcut Annotations

	Guide To Spring Handler Mappings

	Handleradapters In Spring Mvc

	A Custom Data Binder In Spring Mvc

	Geolocation By Ip In Java

	Custom Error Pages With Spring Mvc

	Introduction To Webjars

	Thymeleaf: Custom Layout Dialect

	Spring And Thymeleaf 3: Expressions

	Spring Mvc + Thymeleaf 3.0: New Features

	Apache Tiles Integration With Spring Mvc

	Csrf Protection With Spring Mvc And Thymeleaf

	Using A Custom Spring Mvc’s Handler Interceptor To Manage Sessions

	Cachable Static Assets With Spring Mvc

	Changing Spring Model Parameters With Handler Interceptor

	Introduction To Spring Mvc Handlerinterceptor

	Quick Guide To Spring Controllers

	Quick Guide To Spring Mvc With Velocity

	Spring Mvc And The @modelattribute Annotation

	A Quick Guide To Spring Mvc Matrix Variables

	Javaserver Faces (Jsf) With Spring

	Returning Image/media Data With Spring Mvc

	File Upload With Spring Mvc

	Spring Mvc Content Negotiation

	Exploring Springmvc’s Form Tag Library

	A Guide To The Viewresolver In Spring Mvc

	Introduction To Using Freemarker In Spring Mvc

	A Guide To Spring Redirects

	Spring Mvc Tutorial

















	Jasper Reports With Spring

	Build A Rest Api With Spring 4 And Java Config

	Testing Spring Batch Application - Codecentric.de
	————————————————————————————————————————————————————————————————————————————










	Petri Kainulainen
	—————————————————



	Apache Wicket
	—————————————

	Mocking Spring Beans With Apache Wicket And Mockito

	Wicket Https Tutorial Part One: Creating Secure Webpages

	Wicket Https Tutorial Part Two: Creating Secure Forms

	Wicket Https Tutorial Part Three: Creating A Secure Form Submit From A Non Secure Page




	Apache Hadoop 
	—————————————

	Install And Configure Apache Hadoop To Run In A Pseudo-distributed Mode

	Creating Hadoop Streaming Job With Spring Data Apache Hadoop

	Creating Hadoop Mapreduce Job With Spring Data Apache Hadoop



	 

	Getting Started With Gradle Series 
	——————————————————————————————————



	Spring Social Series 
	————————————————————


	Spring Mvc Test Tutorial 

	Writing Tests For Data Access Code 

	Writing Clean Tests 

	Spring Data Solr Tutorial 




	Spring Batch Series
	———————————————————


	Getting Started
	———————————————

	Spring Batch Tutorial: Introduction

	Spring Batch Tutorial: Getting The Required Dependencies With Maven 

	Spring Batch Tutorial: Getting The Required Dependencies With Gradle 




	Configuration
	—————————————

	Spring Batch 2.2 – Javaconfig Part 1: A Comparison To Xml 

	Spring Batch 2.2 – Javaconfig Part 2: Jobparameters, Executioncontext And Stepscope

	Spring Batch 2.2 – Javaconfig Part 3: Profiles And Environments 

	Spring Batch 2.2 – Javaconfig Part 4: Job Inheritance

	Spring Batch 2.2 – Javaconfig Part 5: Modular Configurations 

	Spring Batch 2.2 – Javaconfig Part 6: Partitioning And Multi-threaded Step 




	Reading Input Data
	——————————————————

	Spring Batch Tutorial: Reading Information From A File 

	Spring Batch Tutorial: Reading Information From A Database

	Spring Batch Tutorial: Creating A Custom Itemreader

	Spring Batch Tutorial: Reading Information From A Rest Api 

	Spring Batch Tutorial: Reading Information From An Excel File




	Writing Output Data
	———————————————————

	Spring Batch Tutorial: Writing Information To A File

	Spring Batch Tutorial: Writing Information To A Database With Jdbc



	Tutorials
	—————————

	Batch Yarn Application

	Restartable Batch Yarn Application 

	Creating A Batch Service

	Spring Batch Tutorial – The Ultimate Guide (Java Code Geeks)







	Spring Data Jpa Series
	——————————————————————


	Spring Data Jpa Tutorial: Introduction

	Spring Data Jpa Tutorial: Getting The Required Dependencies

	Spring Data Jpa Tutorial: Configuration

	Spring Data Jpa Tutorial: Crud

	Spring Data Jpa Tutorial: Introduction To Query Methods

	Spring Data Jpa Tutorial: Creating Database Queries From Method Names

	Spring Data Jpa Tutorial: Creating Database Queries With The @query Annotation 

	Spring Data Jpa Tutorial: Creating Database Queries With Named Queries 

	Spring Data Jpa Tutorial: Creating Database Queries With The Jpa Criteria Api 

	Spring Data Jpa Tutorial: Creating Database Queries With Querydsl 

	Spring Data Jpa Tutorial: Sorting 

	Spring Data Jpa Tutorial: Pagination 

	Spring Data Jpa Tutorial: Auditing - I 

	Spring Data Jpa Tutorial: Auditing - Ii

	Spring Data Jpa Tutorial: Adding Custom Methods Into A Single Repository 

	Spring Data Jpa Tutorial: Adding Custom Methods Into All Repositories 

	Spring Data Jpa Tutorial: Integration Testing 

	Spring Data Jpa Tutorial - Iii

	Spring Data Jpa Tutorial - Ix

	Spring Data Jpa Tutorial: Using Querydsl In A Multi-module Maven Project





	Spring Data Solr Series
	———————————————————————


	Running Solr With Maven

	Spring Data Solr Tutorial: Introduction To Solr 

	Spring Data Solr Tutorial: Configuration

	Spring Data Solr Tutorial: Crud 

	Spring Data Solr Tutorial: Query Methods

	Spring Data Solr Tutorial: Adding Custom Methods To A Single Repository

	Spring Data Solr Tutorial: Dynamic Queries 

	Spring Data Solr Tutorial: Sorting

	Spring Data Solr Tutorial: Pagination

	Spring Data Solr Tutorial: Adding Custom Methods To All Repositories







	Jooq With Spring Series
	———————————————————————

	Using Jooq With Spring: Configuration

	Using Jooq With Spring: Code Generation

	Using Jooq With Spring: Crud

	Using Jooq With Spring: Sorting And Pagination




	Spring From The Trenches Series: War Usage
	——————————————————————————————————————————


	Adding Validation To A Rest Api

	New Like Expressions Of Spring Data Jpa

	Using Environment Specific Cron Expressions With The @scheduled Annotation


	Invoking A Secured Method From A Scheduled Job 

	Using Null Values In Dbunit Datasets

	Resetting Auto Increment Columns Before Each Test Method

	Injecting Property Values Into Configuration Beans

	Returning Runtime Configuration As Json

	Returning Git Commit Information As Json

	Parsing Date And Time Information From A Request Parameter

	Using Type Converters With Spring Mvc

	Creating Pdf Documents With Wkhtmltopdf

	Disabling Cookie Management Of Apache Http Client 4 And Resttemplate





	Maven Series 
	————————————

	Creating A Runnable Binary Distribution With Maven Assembly Plugin 

	Creating Code Coverage Reports For Unit And Integration Tests With The Jacoco Maven Plugin

	Creating Profile Specific Configuration Files With Maven

	Deploying Static And Dynamic Content With Maven Wagon Plugin

	Findbugs Maven Plugin Tutorial 

	Generating Html Documentation From Raml Documents With Maven 

	Integration Testing With Maven 

	Running Integration Tests With Maven

	Running Solr With Maven

	Running Unit Tests With Maven

	Writing Unit Tests With Spock Framework: Creating A Maven Project 







	Multi-tuer Architechture
	————————————————————————

	What Is N-tier Architecture? - Dzone 

	Develop N-tier Applications Using J2ee - Dzone 






	Performance 
	———————————

	Project Management 1/4:sdlc Models & Methodology,agile - Udemy 

	Java Memory Management - Udemy 

	A Comprehensive Introduction To Java Virtual Machine (Jvm) - Udemy 

	Memory Management And Garbage Collection In Java - Lynda 

	Managing Threads In Java - Lynda

	Java Performance Optimization <https://dzone.com/refcardz/java-performance-optimization>

	Optimize The Java Code <https://blog.jooq.org/2015/02/05/top-10-easy-performance-optimisations-in-java/>

	Make Fast Java <http://www.javaworld.com/article/2077647/build-ci-sdlc/make-java-fast--optimize-.html?page=1>






	Vlad Mihalcea Blog 
	——————————————————


	Tips And Best Practices
	————————————————————————————————————

	A Beginner’s Guide To The High-performance-java-persistence Github Repository
	Hibernate Performance Tuning Tips
	14 High-performance Java Persistence Tips
	9 High-performance Tips When Using Mysql With Jpa And Hibernate
	9 High-performance Tips When Using Postgresql With Jpa And Hibernate
	How To Detect The Hibernate N+1 Query Problem During Testing
	A Beginner’s Guide To Sql Injection And How You Should Prevent It
	How To Store Date, Time, And Timestamps In Utc Time Zone With Jdbc And Hibernate
	The Fastest Way To Update A Table Row When Using Hibernate And Oracle
	How To Use Database-specific Or Hibernate-specific Features Without Sacrificing Portability
	Jpa Providers Market Share



	Bootstrapping
	—————————————

	How To Bootstrap Hibernate Without The Persistence.xml Configuration File
	How To Bootstrap Jpa Programmatically Without The Persistence.xml Configuration File
	Jdbc Driver Connection Url Strings
	How To Get Access To Database Table Metadata With Hibernate 5
	How To Get The Entity Mapping To Database Table Binding Metadata From Hibernate




	Mappings: Basic Types
	—————————————————————

	A Beginner’s Guide To Hibernate Types
	A Beginner’s Guide To Jpa And Hibernate Cascade Types
	How To Map Calculated Properties With Jpa And Hibernate @formula Annotation
	How To Map Calculated Properties With Hibernate @generated Annotation
	How To Emulate @createdby And @lastmodifiedby From Spring Data Using The @generatortype Hibernate Annotation
	How To Implement A Custom Basic Type Using Hibernate Usertype
	What’s New In Jpa 2.2 – Java 8 Date And Time Types
	The Best Way To Map A Java 1.8 Optional Entity Attribute With Jpa And Hibernate
	The Best Way To Map An Enum Type With Jpa And Hibernate
	How To Map A Jpa Entity To A View Or Sql Query Using Hibernate
	How To Map The Postgresql Inet Type With Jpa And Hibernate
	How To Map The Java Yearmonth Type With Jpa And Hibernate
	How To Map Java.time.year And Java.time.month With Jpa And Hibernate


	Json
	————

	The Hibernate-types Open-source Project Bring Extra Hibernate Types (E.g. Json, Array)
	How To Map Json Objects Using Generic Hibernate Types
	How To Store Schema-less Eav (Entity-attribute-value) Data Using Json And Hibernate
	How To Map Json Collections Using Jpa And Hibernate
	How To Customize The Jackson Objectmapper Used By Hibernate-types
	How To Customize The Json Serializer Used By Hibernate-types
	How To Fix The Hibernate “no Dialect Mapping For Jdbc Type” Issue




	Array
	—————

	How To Map Java And Sql Arrays With Jpa And Hibernate
	Hibernate Hsqldb Array Type
	Equals And Hashcode
	The Best Way To Implement Equals, Hashcode, And Tostring With Jpa And Hibernate
	How To Implement Equals And Hashcode Using The Entity Identifier (Primary Key)
	How To Implement Equals And Hashcode Using The Entity Natural Identifier




	Relationships
	—————————————

	The Best Way To Map A @onetoone Relationship With Jpa And Hibernate
	The Best Way To Map A @onetomany Relationship With Jpa And Hibernate
	The Best Way To Use The @manytomany Annotation With Jpa And Hibernate
	The Best Way To Map A Many-to-many Association With Extra Columns When Using Jpa And Hibernate
	The Best Way To Map A Composite Primary Key With Jpa And Hibernate
	How To Map A Composite Identifier Using An Automatically @generatedvalue With Jpa And Hibernate
	How To Synchronize Bidirectional Entity Associations With Jpa And Hibernate
	How To Map A @manytoone Association Using A Non-primary Key Column
	How To Customize An Entity Association Join On Clause With Hibernate @joinformula
	How To Map A Jpa @manytoone Relationship To A Sql Query Using The Hibernate @joinformula Annotation
	How To Optimize Unidirectional Collections With Jpa And Hibernate
	How Do Set And List Collections Behave With Jpa And Hibernate



	Advanced Mapping Techniques
	———————————————————————————

	Fluent Api Entity Building With Jpa And Hibernate
	How To Map An Immutable Entity With Jpa And Hibernate
	How To Map The Latest Child Of A Parent Entity Using Hibernate @joinformula
	How To Update Only A Subset Of Entity Attributes Using Jpa And Hibernate
	How To Use External Xml Mappings Files With Jpa And Hibernate
	How To Encrypt And Decrypt Data With Hibernate
	The Best Way To Soft Delete With Hibernate
	How To Fix “wrong Column Type Encountered” Schema-validation Errors With Jpa And Hibernate
	How To Audit Entity Modifications Using The Jpa @entitylisteners, @embedded, And @embeddable Annotations
	How To Use @prepersist And @preupdate On Embeddable With Jpa And Hibernate
	How To Map Camelcase Properties To Snake_case Column Names With Hibernate




	Identifiers
	———————————

	A Beginner’s Guide To Natural And Surrogate Database Keys
	The Best Way To Map A @naturalid Business Key With Jpa And Hibernate
	The Hi/lo Algorithm
	Hibernate And Uuid Identifiers
	How To Use A Jvm Or Database Auto-generated Uuid Identifier With Jpa And Hibernate
	Hibernate Identity, Sequence, And Table (Sequence) Generator
	A Beginner’s Guide To Hibernate Enhanced Identifier Generators
	Hibernate Pooled And Pooled-lo Identifier Generators
	Why You Should Never Use The Table Identifier Generator With Jpa And Hibernate
	Why Should Not Use The Auto Jpa Generationtype With Mysql And Hibernate
	How To Replace The Table Identifier Generator With Either Sequence Or Identity In A Portable Way
	Postgresql Serial Column And Hibernate Identity Generator
	How To Combine The Hibernate Assigned Generator With A Sequence Or An Identity Column
	How To Implement A Custom String-based Sequence Identifier Generator With Hibernate
	Mariadb 10.3 Supports Database Sequences




	Inheritance
	———————————

	The Best Way To Use Entity Inheritance With Jpa And Hibernate
	The Best Way To Map The Single_table Inheritance With Jpa And Hibernate
	The Best Way To Map The @discriminatorcolumn With Jpa And Hibernate
	How To Inherit Properties From A Base Class Entity Using @mappedsuperclass With Jpa And 

	Hibernate
	———————————
	How To Order Entity Subclasses By Their Class Type Using Jpa And Hibernate
	Connection Management
	The Simple Scalability Equation
	The Anatomy Of Connection Pooling
	Why You Should Use Flexypool
	How To Monitor Your Connection Pool With Flexypool
	How To Monitor A Java Ee Datasource
	Why You Should Always Use Hibernate.connection.provider_disables_autocommit For Resource-local Jpa Transactions
	How Does Aggressive Connection Release Work In Hibernate


	Persistence Context
	———————————————————

	A Beginner’s Guide To Jpa/hibernate Entity State Transitions
	A Beginner’s Guide To Flush Strategies In Jpa And Hibernate
	How Does Persist And Merge Work In Jpa
	How To Merge Entity Collections With Jpa And Hibernate
	How Does Auto Flush Strategy Work In Hibernate
	How Do Jpa And Hibernate Define The Auto Flush Mode
	A Beginner’s Guide To Hibernate Flush Operation Order
	The Best Way To Clone Or Duplicate An Entity With Jpa And Hibernate
	The Anatomy Of Hibernate Dirty Checking Mechanism
	How To Customize Hibernate Dirty Checking Mechanism
	How To Enable Bytecode Enhancement Dirty Checking In Hibernate
	How Does The Bytecode Enhancement Dirty Checking Mechanism Work In Hibernate 4.3



	Fetching
	————————

	How Do Find And Getreference Entitymanager Methods Work When Using Jpa And Hibernate
	A Beginner’s Guide To Hibernate Fetching Strategies
	Eager Fetching Is A Code Smell
	The Best Way To Map A Projection Query To A Dto (Data Transfer Object) With Jpa And Hibernate
	The Best Way To Handle The Lazyinitializationexception
	The Best Way To Lazy Load Entity Attributes Using Jpa And Hibernate
	Resultset Statement Fetching With Jdbc And Hibernate
	How To Fetch Entities Multiple Levels Deep With Hibernate
	The Open Session In View Anti-pattern
	The Hibernate.enable_lazy_load_no_trans Anti-pattern
	How Does Mysql Result Set Streaming Perform Vs Fetching The Whole Jdbc Resultset At Once
	How Does A Jpa Proxy Work And How To Unproxy It With Hibernate
	The Best Way To Fix The Hibernate “hhh000104: Firstresult/maxresults Specified With Collection Fetch; Applying In Memory!” Warning Message





	Concurrency Control
	———————————————————

	Data Knowledge Stack
	A Beginner’s Guide To Acid And Database Transactions
	How Does Mvcc (Multi-version Concurrency Control) Work
	How Do Postgresql Advisory Locks Work
	A Beginner’s Guide To Database Locking And The Lost Update Phenomena
	How To Prevent Lost Updates In Long Conversations
	A Beginner’s Guide To Dirty Read Anomaly
	A Beginner’s Guide To Non-repeatable Read Anomaly
	A Beginner’s Guide To Phantom Read Anomaly
	A Beginner’s Guide To Read And Write Skew Phenomena
	A Beginner’s Guide To The Phantom Read Anomaly, And How It Differs Between 2pl And Mvcc
	How Does Database Pessimistic Locking Interact With Insert, Update, And Delete Sql Statements
	How Do Upsert And Merge Work In Oracle, Sql Server, Postgresql And Mysql
	Logical Vs Physical Clock Optimistic Locking
	How To Retry Jpa Transactions After An Optimisticlockexception
	How Does Hibernate Guarantee Application-level Repeatable Reads
	Hibernate Collections Optimistic Locking
	How To Address The Optimisticlockexception In Jpa And Hibernate
	How To Prevent Optimisticlockexception With Hibernate Versionless Optimistic Locking
	A Beginner’s Guide To Transaction Isolation Levels In Enterprise Java
	A Beginner’s Guide To Java Persistence Locking
	How Does Lockmodetype.optimistic Work In Jpa And Hibernate
	How To Fix Optimistic Locking Race Conditions With Pessimistic Locking
	How Does Lockmodetype.optimistic_force_increment Work In Jpa And Hibernate
	How Does Lockmodetype.pessimistic_force_increment Work In Jpa And Hibernate
	How Do Lockmodetype.pessimistic_read And Lockmodetype.pessimistic_write Work In Jpa And Hibernate
	How Does Cascadetype.lock Works In Jpa And Hibernate
	How To Increment The Parent Entity Version Whenever A Child Entity Gets Modified With Jpa And Hibernate
	Spring Read-only Transaction Hibernate Optimization




	Batching
	————————


	Batch Processing Best Practices
	The Best Way To Do Batch Processing With Jpa And Hibernate
	How To Batch Insert And Update Statements With Hibernate
	How To Batch Delete Statements With Hibernate
	How To Customize The Jdbc Batch Size For Each Persistence Context With Hibernate
	How To Find Which Statement Failed In A Jdbc Batch Update
	How To Optimize The Merge Operation Using Update While Batching With Jpa And Hibernate
	How To Enable Multi-row Inserts With The Postgresql Rewritebatchedinserts Configuration Property




	Queries
	———————

	How To Improve Statement Caching Efficiency With In Clause Parameter Padding
	A Beginner’s Guide To The Hibernate Jpql And Native Query Plan Cache
	How To Optimize Jpql And Criteria Api Query Plans With Hibernate Statistics
	The Best Way To Use The Jpql Distinct Keyword With Jpa And Hibernate
	How To Join Unrelated Entities With Jpa And Hibernate
	How To Resolve The Hibernate Global Database Schema And Catalog For Native Sql Queries
	How To Map Table Rows To Columns Using Sql Pivot Or Case Expressions
	The Jpa Entitymanager Createnativequery Is A Magic Wand
	Why You Should Use The Hibernate Resulttransformer To Customize Result Set Mappings
	Why You Should Definitely Learn Sql Window Functions
	Query Pagination With Jpa And Hibernate
	What’s New In Jpa 2.2 – Stream The Result Of A Query Execution
	How To Get The Actual Execution Plan For An Oracle Sql Query Using Hibernate Query Hints
	How Do Solve The Postgresql :: Cast Operator Issue With Jpa And Hibernate
	The Best Way To Use Sql Functions In Jpql Or Criteria Api Queries With Jpa And Hibernate
	How To Query Parent Rows When All Children Must Match The Filtering Criteria With Sql And Hibernate
	How To Query By Entity Type Using Jpa Criteria Api


	Criteria Queries
	————————————————

	Jpa Criteria Api Bulk Update And Delete
	Why You Should Always Check The Sql Statements Generated By Criteria Api
	The Performance Penalty Of Class.forname When Parsing Jpql And Criteria Queries
	How Does Hibernate Handle Jpa Criteria Api Literals
	Stored Procedures
	How To Call Oracle Stored Procedures And Functions With Jpa And Hibernate
	How To Call Sql Server Stored Procedures And Functions With Jpa And Hibernate
	How To Call Postgresql Functions (Stored Procedures) With Jpa And Hibernate
	How To Call Mysql Stored Procedures And Functions With Jpa And Hibernate



	Caching
	———————

	A Beginner’s Guide To Cache Synchronization Strategies
	Things To Consider Before Jumping To Enterprise Caching
	Caching Best Practices
	How Does Hibernate Store Second-level Cache Entries
	How Does Hibernate Read_only Cacheconcurrencystrategy Work
	How Does Hibernate Nonstrict_read_write Cacheconcurrencystrategy Work
	How Does Hibernate Read_write Cacheconcurrencystrategy Work
	How Does Hibernate Transactional Cacheconcurrencystrategy Work
	How Does Hibernate Collection Cache Work
	How Does Hibernate Query Cache Work
	How To Use The Hibernate Query Cache For Dto Projections
	How To Avoid The Hibernate Query Cache N+1 Issue
	How To Cache Non-existing Entity Fetch Results With Jpa And Hibernate


	Audit
	—————

	A Beginner’s Guide To Cdc (Change Data Capture)
	How To Extract Change Data Events From Mysql To Kafka Using Debezium
	The Best Way To Implement An Audit Log Using Hibernate Envers
	Multitenancy
	A Beginner’s Guide To Database Multitenancy
	Hibernate Database Catalog Multitenancy
	Hibernate Database Schema Multitenancy


	Testing
	———————

	The Minimal Configuration For Testing Hibernate
	Hibernate Integration Testing Strategies
	How To Run Database Integration Tests 20 Times Faster
	How To Run Integration Tests At Warp Speed Using Docker And Tmpfs
	The Best Way To Log Jdbc Statements
	The Best Way To Detect Database Connection Leaks
	How To Install Db2 Express-c On Docker And Set Up The Jdbc Connection Properties
	How To Get Started With Cockroachdb And Hibernate









	———————————————————————————————————————

	Good Websites 
	—————————————

	I.   Mkyong.com
	Ii.  Codejava.net
	Iii. Javacodegeeks.com
	———————————————————————————————————————





	Books 
	—————


	High-performance Java Persistence - Vlad Mihalcea

	Restful Java With Jax-rs 2.0  - O’reilly (Designing Distributed Web Services)

	Rest Assured With Jax-rs - Git-book

	High Performance Java Persistence - Vlad Mihalcea

